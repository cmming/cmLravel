        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{url('/topic/'.$topic->id.'/submit')}}" method="POST">
                        {{csrf_field()}}
                        @foreach($myposts as $mypost)
                            <div class="checkbox">
                                <label>
                                    <input type="checkbox" name="post_ids[]" value="{{$mypost->id}}">
                                    {!! str_limit($mypost->title,10) !!}
                                </label>
                            </div>
                        @endforeach
            ...
            <div class="tab-content">
                ...
                @foreach($posts as $post)
                        <div class="blog-post" style="margin-top: 30px">
                            <p class=""><a
                                        href="{{url('/user/'.$post->user->id)}} ">{{$post->user->name}}</a> {{$post->created_at->diffForHumans()}}
                            </p>
                            <p class=""><a href="{{url('/posts/'.$post->id)}}">{!! str_limit($post->title,10) !!}</a>
                            </p>
